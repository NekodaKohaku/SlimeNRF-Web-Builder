...
# Per-MCU hardware facts (AIN maps, pin constraints) live in docs/mcus.json, which the web UI
# fetches too. Single source of truth: the pin list the configurator offers for the ADC and the
# AIN number this script emits are derived from the same table and cannot drift apart.
MCUS = json.loads(
    (pathlib.Path(__file__).resolve().parents[2] / "docs" / "mcus.json").read_text(encoding="utf-8")
)["mcus"]
...
def ain_map(mcu):
    m = MCUS.get(mcu)
    if not m:
        sys.exit(f"unknown mcu: {mcu!r} (add it to docs/mcus.json)")
    return m["ain"]
...
def build_nrf52(bus, pins, opts, ain):
...
def build_nrf54l(bus, pins, opts, ain):
    mc = (opts or {}).get("mag_conn", "none")
    lt = (opts or {}).get("led_type", "single")
    led_on = bool(pins.get("led")) and pins.get("led") != "none"
    ...
    if (opts or {}).get("adc") == "external" and (opts or {}).get("adc_pin") in ain:
...
def main():
    ...
    if cfg.get("type") == "receiver":
        ...
        L = build_nrf54l(bus, pins, opts, ain_map(mcu))
    else:
        L = build_nrf52(bus, pins, opts, ain_map(mcu))
    hdr = []
